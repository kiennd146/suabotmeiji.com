<?php
include_once(dirname(__FILE__).'/CsFilterProductRightClass.php');

class CsFilterProductRight extends Module
{
	private $_html;
	...
	{
	 	$this->name = 'csfilterproductright';
	 	$this->tab = 'MyBlocks';
	 	$this->version = '1.0';
		...
		$this->displayName = $this->l('CS Filter Product Right');
		$this->description = $this->l('Add Filter Products right on the homepage');
		$this->confirmUninstall = $this->l('Are you sure that you want to delete your csfilterproductright?');
	...
	public function install()
	{
		...
		if (!Db::getInstance()->Execute('CREATE TABLE '._DB_PREFIX_.'csfilterproductright (`id_tab` int(10) unsigned NOT NULL AUTO_INCREMENT, `product_type` varchar(255), `position` int(10) unsigned DEFAULT \'0\',`hook` int(10) unsigned,`display` tinyint(1) NOT NULL DEFAULT \'1\', PRIMARY KEY (`id_tab`)) ENGINE=InnoDB default CHARSET=utf8'))
	 		return false;
		if (!Db::getInstance()->Execute('CREATE TABLE '._DB_PREFIX_.'csfilterproductright_shop (`id_tab` int(10) unsigned NOT NULL ,`id_shop` int(10) unsigned NOT NULL,`product_type` varchar(255), `position` int(10) unsigned DEFAULT \'0\', `display` tinyint(1) NOT NULL DEFAULT \'1\', PRIMARY KEY (`id_tab`,`id_shop`)) ENGINE=InnoDB default CHARSET=utf8'))
	 		return false;
		if (!Db::getInstance()->Execute('CREATE TABLE '._DB_PREFIX_.'csfilterproductright_lang (`id_tab` int(10) unsigned NOT NULL, `id_lang` int(10) unsigned NOT NULL,`id_shop` int(10) unsigned NOT NULL, `title` varchar(255) NOT NULL DEFAULT \'\', PRIMARY KEY (`id_tab`,`id_lang`,`id_shop`)) ENGINE=InnoDB default CHARSET=utf8'))
	 		return false;
	 	return true;
	}
	...
	public function uninstall()
	{
	 	if (parent::uninstall() == false)
	 		return false;
		if (!Db::getInstance()->Execute('DROP TABLE '._DB_PREFIX_.'csfilterproductright') OR !Db::getInstance()->Execute('DROP TABLE '._DB_PREFIX_.'csfilterproductright_shop') OR  !Db::getInstance()->Execute('DROP TABLE '._DB_PREFIX_.'csfilterproductright_lang'))
	 		return false;
		$this->_clearCache('csfilterproductright.tpl');
	 	return true;
	}
	...
	private function _postProcess()
	{
		...
		{
			$tab = new csfilterproductrightClass(Tools::getValue('id_tab'));
			$tab->copyFromPost();
	...
	private function getTabs($active = null) //case in : allshop show shop default
	{
		...
		$id_shop = $this->context->shop->id;
	 	if (!$result = Db::getInstance()->ExecuteS(
			...
			FROM `'._DB_PREFIX_.'csfilterproductright` t
			LEFT JOIN `'._DB_PREFIX_.'csfilterproductright_shop` ts ON (ts.`id_tab` = t.`id_tab` )
			LEFT JOIN `'._DB_PREFIX_.'csfilterproductright_lang` tl ON (t.`id_tab` = tl.`id_tab` '.( $id_shop ? 'AND tl.`id_shop` = '.$id_shop : ' ' ).') 
	...
	private function _displayUpdateForm()
	{
		...
		$tab = new csfilterproductrightClass((int)Tools::getValue('id_tab'));
	 	// Language 
	 	$defaultLanguage = (int)(Configuration::get('PS_LANG_DEFAULT'));
		$languages = Language::getLanguages(false);
		$divLangName = 'titlediv';
	...
	private function getTabsDisplay($nb = 10,$id_shop,$hook)
	{
		...
		$id_hook = Hook::getIdByName($hook);
	 	$results = Db::getInstance()->ExecuteS(
					'SELECT ts.`id_tab` FROM `'._DB_PREFIX_.'csfilterproductright_shop` ts
					LEFT JOIN `'._DB_PREFIX_.'csfilterproductright` t ON (ts.id_tab = t.id_tab)
		...
		{
			$temp = new csfilterproductrightClass($row['id_tab']);
	...
	{	
		...
		{
			$this->context->controller->addCss($this->_path.'css/csfilterproductright.css');
		}
	}
	...
	public function hookDisplayHome()
	{
		...
		$this->context = Context::getContext();
		$id_shop = $this->context->shop->id;
		$tabs = $this->getTabsDisplay(11,$id_shop,'displayHome');
		$smarty->assign(array(
			'tabs' => $tabs
		));
		return $this->display(__FILE__, 'csfilterproductright.tpl');
	}
	public function hookHomeBottom()
	{
		...
		$this->context = Context::getContext();
		$id_shop = $this->context->shop->id;
		$tabs = $this->getTabsDisplay(11,$id_shop,'homebottom');
		$smarty->assign(array(
			'tabs' => $tabs
		));
		return $this->display(__FILE__, 'csfilterproductright.tpl');
