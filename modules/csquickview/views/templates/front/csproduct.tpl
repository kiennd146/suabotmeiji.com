...
var doesntExistNoMoreBut = '{l s='with those attributes but is available with others' js=1 mod='csquickview'}';
var uploading_in_progress = '{l s='Uploading in progress, please wait...' js=1}';
var fieldRequired = '{l s='Please fill in all required fields, then save the customization.' js=1 mod='csquickview'}';
{if isset($groups)}
	// Combinations
...
<div id="primary_block" class="clearfix">
	...
	<div id="admin-action">
		<p>{l s='This product is not visible to your customers.' mod='csquickview'}
		<input type="hidden" id="admin-action-product-id" value="{$product->id}" />
		<input type="submit" value="{l s='Publish' mod='csquickview'}" class="exclusive" onclick="submitPublishProduct('{$base_dir}{$smarty.get.ad|escape:'htmlall':'UTF-8'}', 0, '{$smarty.get.adtoken|escape:'htmlall':'UTF-8'}')"/>
		<input type="submit" value="{l s='Back' mod='csquickview'}" class="exclusive" onclick="submitPublishProduct('{$base_dir}{$smarty.get.ad|escape:'htmlall':'UTF-8'}', 1, '{$smarty.get.adtoken|escape:'htmlall':'UTF-8'}')"/>
		</p>
	...
	<div id="pb-left-column">
		...
		<div id="short_description_block">
			...
			<p class="buttons_bottom_block"><a href="javascript:{ldelim}{rdelim}" class="button">{l s='More details' mod='csquickview'}</a></p>
			{/if}
			{if $packItems|@count > 0}
			<div class="short_description_pack">
				<h3>{l s='Pack content' mod='csquickview'}</h3>
		...
		<!-- colors -->
		<div id="color_picker">
			<p>{l s='Pick a color:' js=1 mod='csquickview'}</p>
			<div class="clear"></div>
		...
		<!-- add to cart form-->
		<form id="buy_block" target="_parent" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
			...
			<p id="product_reference" {if isset($groups) OR !$product->reference}style="display: none;"{/if}>
				<label for="product_reference">{l s='Reference:' mod='csquickview'} </label>
				<span class="editable">{$product->reference|escape:'htmlall':'UTF-8'}</span>
			</p>

			<!-- quantity wanted -->
			<p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				<label>{l s='Quantity:' mod='csquickview'}</label>
			...
			<p id="minimal_quantity_wanted_p"{if $product->minimal_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				{l s='This product is not sold individually. You must select at least' mod='csquickview'} <b id="minimal_quantity_label">{$product->minimal_quantity}</b> {l s='quantity for this product.' mod='csquickview'}
			</p>
			...
			<p id="availability_statut"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) OR ($product->quantity > 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				<span id="availability_label">{l s='Availability:' mod='csquickview'}</span>
				<span id="availability_value"{if $product->quantity <= 0} class="warning_inline"{/if}>
				{if $product->quantity <= 0}{if $allow_oosp}{$product->available_later}{else}{l s='This product is no longer in stock' mod='csquickview'}{/if}{else}{$product->available_now}{/if}
				</span>
			</p>
			...
			<p id="pQuantityAvailable"{if $product->quantity <= 0} style="display: none;"{/if}>
				...
				<span {if $product->quantity > 1} style="display: none;"{/if} id="quantityAvailableTxt">{l s='item in stock' mod='csquickview'}</span>
				<span {if $product->quantity == 1} style="display: none;"{/if} id="quantityAvailableTxtMultiple">{l s='items in stock' mod='csquickview'}</span>
			</p>
			{/if}

			
			<p class="warning_inline" id="last_quantities"{if ($product->quantity > $last_qties OR $product->quantity <= 0) OR $allow_oosp OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none"{/if} >{l s='Warning: Last items in stock!' mod='csquickview'}</p>
		</div>
		...
		<div class="content_prices clearfix">
			...
			<p class="online_only">{l s='Online only' mod='csquickview'}</p>
			{/if}
			...
			<div class="price">
				...
				{elseif $product->specificPrice AND $product->specificPrice.reduction AND $productPriceWithoutReduction > $productPrice}
					<span class="discount">{l s='Reduced price!' mod='csquickview'}</span>
				{/if}
				{if $priceDisplay == 2}
					<br />
					<span id="pretaxe_price"><span id="pretaxe_price_display">{convertPrice price=$product->getPrice(false, $smarty.const.NULL)}</span>&nbsp;{l s='tax excl.' mod='csquickview'}</span>
				{/if}
			</div>
			...
			{if $product->ecotax != 0}
				<p class="price-ecotax">{l s='include'} <span id="ecotax_price_display">{if $priceDisplay == 2}{$ecotax_tax_exc|convertAndFormatPrice}{else}{$ecotax_tax_inc|convertAndFormatPrice}{/if}</span> {l s='for green tax'}
					{if $product->specificPrice AND $product->specificPrice.reduction}
					<br />{l s='(not impacted by the discount)' mod='csquickview'}
					{/if}
				</p>
			{/if}
			...
			{if (!$allow_oosp && $product->quantity <= 0) OR !$product->available_for_order OR (isset($restricted_country_mode) AND $restricted_country_mode) OR $PS_CATALOG_MODE}
				<span class="exclusive">
					<span></span>
					{l s='Add to cart' mod='csquickview'}
				</span>
			{else}
				<p id="add_to_cart" class="buttons_bottom_block">
					<span></span>
					<input type="submit" name="Submit" onclick="return closeFancy()" value="{l s='Add to cart' mod='csquickview'}" class="exclusive" />
				</p>
			{/if}
...
<ul class="idTabs clearfix">
	<li><a href="#discount" style="cursor: pointer" class="selected">{l s='Quantity discount' mod='csquickview'}</a></li>
</ul>
<div id="quantityDiscount">
	<table class="std">
        <thead>
            <tr>
                <th>{l s='product' mod='csquickview'}</th>
                <th>{l s='from (qty)' mod='csquickview'}</th>
                <th>{l s='discount' mod='csquickview'}</th>
            </tr>
        </thead>
		<tbody>
